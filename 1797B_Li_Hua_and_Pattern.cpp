...
 
 
bool is_valid(ll x, ll n, ll k, vector<vector<ll>>&V){
    if(V[0][0]>x){
        return false;
    }
 
...
 
        if(a_i<x){
            ...
            while(a_i>x){
                ...
                pair<ll, ll> pie= pq.top();
                pq.pop();
                ll c_j= pie.first;
                ll vac= pie.second;
                ll can_shift= min(vac, a_i-x);
                ll cost= can_shift*(b_i+c_j);
...
 
    ...
    for(int i=0; i<n; i++){
        for(int j=0; j<n; j++){
            if(V[i][j]== V[n-1-i][n-1-j]) continue;
            else{
                //cout<<i<<" "<<j<<'\n';
                if(left<=0){
                    pno
