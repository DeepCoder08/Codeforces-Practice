...
bool is_valid(string s, string t, int k){
    //check removing all the k length substrings from s
    if(k==0) return true;
 
 
    int n= s.length();
    if(n-k< t.length()) return false;
 
    for(int i=0; i<n-k+1; i++){
        string t1="";
        t1+= s.substr(0, i);
        if(i+k<n) t1+= s.substr(i+k, n-i-k);
        if(isSubsequence(t, t1)) return true;
    }
 
    return false;
 
