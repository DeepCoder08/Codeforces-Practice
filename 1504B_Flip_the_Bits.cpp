...
 
    vector<int>opposite_bits(n+1, 0);
    for(int i=1; i<=n; i++){
        opposite_bits[i]= opposite_bits[i-1]+ ((a[i-1]!=b[i-1])? 1:0);
    }
 
    // for(int i=0; i<=n; i++){
    //     cout<<opposite_bits[i]<<" ";
    // }
    // cout<<'\n';
 
    int last=-1;
...
 
        if(ones== zeros){
            enter=1;
            //cout<<i<<" ";
            int opp= opposite_bits[i+1]-opposite_bits[last+1];
            int len= i-last;
            //cout<<opp<<" "<<len<<endl;
            if(!(opp==0 || opp==len)){
                pno
...
 
    int x= opposite_bits[last+1];
    for(int i=last+1; i<n; i++){
        if(opposite_bits[i+1]!=x){
            pno
            return;
        }
    }
